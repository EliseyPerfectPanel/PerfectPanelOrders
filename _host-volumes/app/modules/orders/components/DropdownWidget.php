        ...
        if(!empty($this->items)){
            ...
            if(is_array($this->items)) {
                ...
                $links[] = [
                    ...
                    'active' => !empty($this->addGetParam) ? function ($item, $hasActiveChild, $isItemActive, $widget) {
                        return yii::$app->request->get($this->addGetParam) !== NULL ? false : true;
                    } : null
                ];

                foreach ($this->items as $key => $val) {
                    $links[] = ['label' => $val, 'url' => ArrayHelper::merge($this->url, !empty($this->addGetParam) ? [$this->addGetParam => $key] : null)];
                }
